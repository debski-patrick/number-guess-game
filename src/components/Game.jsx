...
const Game = (props) => {
  ...
  const [moves, setMoves] = useState(store.getState().moves.moves ? store.getState().moves.moves : []);
  const [difficulty, setDifficulty] = useState(store.getState().equationLength);
  // const [gameStatus, setGameStatus] = useState(null);
  const gameStatus = typeof window !== 'undefined' && JSON.parse(localStorage.getItem(`Guess-Status-${difficulty}`));
  ...
  const [status, setStatus] = useState(store.getState().status ? store.getState().status : "playing");
  const [popup, setPopup] = useState();

  // useEffect(() => {
  //   setGameStatus(JSON.parse(localStorage.getItem(`Guess-Status-${difficulty}`)));
  //   console.log(JSON.parse(localStorage.getItem(`Guess-Status-${difficulty}`)));
